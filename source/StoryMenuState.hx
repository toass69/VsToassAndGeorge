...
class StoryMenuState extends MusicBeatState
{
	...
	var weekData:Array<Dynamic> = [
		['Toass', 'Persistence Of Cosmos', 'Suppression'],
		['Bailing', 'Cader', 'Hills'],
		['Splitathon']
	];

	var actualWeeks:Array<Dynamic> = [0, 1, 2];
	...
	override function update(elapsed:Float)
	{
		...
		else
		{
			...
			grpWeekText.members[0].week.loadGraphic(Paths.image('storymenu/week' + 0));
			grpWeekText.members[1].visible = false;
			grpWeekText.members[2].week.loadGraphic(Paths.image('storymenu/week' + 2));
		}
	...
	function selectWeek()
	{
		...
		{
			if (FlxG.save.data.tristanProgress == "pending play" && (curWeek > 2 || curWeek == 0) && curWeek != 10)
			{
				FlxG.sound.play(Paths.sound('cancelMenu'));
				return;
			}
	...
	function updateBgColor()
	{
		...
		{
			case 0:
				targetColor = 0xFF7F00FF; // Purple for Dave week
			case 1:
				targetColor = 0xFFFF0000; // Red for Bambi week
			case 2:
				targetColor = 0xFF0094FF; // Blue for Finale week
			default:
				targetColor = 0xFF0094FF; // Default blue
		}

		FlxTween.color(yellowBG, 0.4, yellowBG.color, targetColor, {ease: FlxEase.quadInOut});
	}
	...
	{
		var path:String = Paths.image("blank", "shared");
		var position:FlxPoint = new FlxPoint(600, 200);
