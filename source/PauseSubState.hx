...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		// Botplay indicator
		var botplayText:FlxText = new FlxText(20, 15 + 64, 0, "", 32);
		botplayText.text = "BOTPLAY";
		botplayText.scrollFactor.set();
		botplayText.setFormat(Paths.font('vcr.ttf'), 32);
		botplayText.updateHitbox();
		botplayText.x = FlxG.width - (botplayText.width + 20);
		botplayText.color = FlxColor.LIME;
		botplayText.visible = PlayState.botplayMode;
		botplayText.ID = 999; // Special ID to find it later
		add(botplayText);

		if (PlayState.botplayMode)
		{
			botplayText.alpha = 0;
		...
		if (accepted)
		{
			...
			switch (daSelected)
			{
				...
				case "Restart Song":
					FlxG.resetState();
				case "Toggle Botplay":
					PlayState.botplayMode = !PlayState.botplayMode;
					
					// Update the botplay indicator
					for (member in members)
					{
						if (Std.isOfType(member, FlxText))
						{
							var txt:FlxText = cast member;
							if (txt.ID == 999) // Our botplay text
							{
								txt.visible = PlayState.botplayMode;
								if (PlayState.botplayMode)
								{
									txt.alpha = 0;
					...
					// Don't close yet - let player see the change
				case "Exit to menu":
